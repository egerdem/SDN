...
class DelayLine(object):
    def __init__(self, start, end):
        ...
        self.buffer = col.deque(np.zeros(self.length, dtype= np.float))
...
if __name__ == "__main__":
    ...
    # mic = Microphone(np.array([2, 2, 1.5]))
    #
    # src = Source(np.array([2.5, 3.5, 2]))
    #
    # shoebox_room = Room(6, 7, 4,
    #                     0.3,
    #                     mic,
    #                     src)
    ...
    read_write.read_sound_file(".\\samples\\Archim-5-anecoic.wav", src)
    ...
    read_write.write_sound_file("testing_Archim-5-anecoic.wav", mic)
