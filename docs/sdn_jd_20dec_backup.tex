...
\section{Investigation of Energy Loss in SDN}
\label{sec:energy_drop_problem}
This work investigates the root cause of the sudden energy drop in the early reflections in SDN and proposes an altered source injection approach to mitigate the problem.
To demonstrate the issue, we analyze how source pressure is distributed through the network by tracking the injected energy into the system.
...
where $\mathbf{S}$ is the $(N-1) \times (N-1)$ scattering matrix,  $1/2$ is the constant coefficient scaling the source pressure $p_{S}$ received at node $k$, and $\mathbf{p_k^+}$ is the vector of incoming waves from neighboring nodes, without any source contribution lumped. The augmented matrix in the left-hand side now combines:
\begin{itemize}
    \item The scattering matrix $\mathbf{S}$ that redistributes incoming waves
    \item The source injection vector $\frac{1}{2}\mathbf{1}_{N-1}$ that lumps and distributes source pressure to neighboring directions
    \item The pressure summation vector $\frac{2}{N-1}\mathbf{1}^T$ that calculates the node pressure
\end{itemize}

The source injection vector $\frac{1}{2}\mathbf{1}_{N-1}$ distributes the source signal uniformly across all incoming neighbor directions. The uniform injection simulates simultaneous reflections from all walls although the source wave has not actually reached any wall surfaces yet. Further explanation is provided in Section~\ref{sec:physical_relevance}.

Right hand side of Eq.~\ref{eq:augmented_matrix} shows the resultant pressure value of each $(N-1)$ outgoing wave as the column vector $\mathbf{p_k^-}$, and the overall pressure state of node $k$ as $p_k$, which is sent to the receiver. 
The receiver pressure is formed by scaled sum of the outgoing (reflected) waves, where the sum is equal to the sum of incoming waves due to the unitary structure of the scattering matrix. 

As pressure calculations are done for each time step $n$, outgoing pressures become the incoming pressures for other nodes at next time steps, and the process is repeated until the simulation ends. 
The first-order paths involve direct source-to-node propagation without node-to-node scattering.
Thus, influence of the scattering network and source injection start at second-order reflections and beyond, when signals begin to propagate between nodes.

To demonstrate how energy reaching the receiver is affected by the pressure equations, we consider a Dirac impulse with unit amplitude (i.e., $p_{S}[n] = 1$ for $n = 0$ and $p_{S}[n] = 0$ for $n \neq 0$) as the source signal. Simple cuboid room with $N=6$ (number of walls) is used throughout the paper. To isolate the problem only to the source injection and scattering operations, wall absorption and distance-based attenuations are removed (set to 1) in subsequent pressure analyses.

\textbf{For the first order paths} $[source-node_{\text{k}}-mic]$, incoming waves from other nodes to node \(k\) are zero, $\mathbf{p}_k^+ = \mathbf{0}$. Since there are no separate source to node connections (i.e delay lines), the source signal is uniformly injected into each incoming neighbor wave $\tilde p_{ki}^+$, with a factor of $\frac{1}{2}$. 
Then the intended source pressure at the node, $p_{k}$ (i.e the overall pressure state of node \(k\) at that time instant), is calculated as equal to the input dirac impulse,  $p_k=1=p_{S}[0]$, which is the expected pressure at the recevier, in the absence of any absorption or attenuation:
...
=
\begin{bmatrix}
\frac{1}{2}\mathbf{1}_{N-1} \\
1
\end{bmatrix}
\end{equation}

Eq.~\ref{eq:matrix_formulation} also shows that the outgoing wave vector is not changed by the scattering operation and stayed as $\mathbf{p}_k^- = \mathbf{p}_k^+ = \frac{1}{2}\mathbf{1}_{N-1}$,
due to the energy-preserving structure of the scattering matrix. This means that each neighbor node receives the same portion of the source signal,  $\frac{p_S=1}{2}$, reflected from node \(k\). 
%Generalizing this, any input vector $\mathbf{p}_k^+$ formed by same elements will go over a unitary transformation by S as the scattering matrix has circularly rotated rows, averaging the input vector.

\textbf{For the second order paths,} consider the outgoing wave in the direction of node \(j\) which is the closest node to the current node \(k\). 
By the time the reflected wave with amplitude \(p_{kj}^-(n)= \frac{1}{2}\) reaches node \(j\), there are no other non-zero incoming waves present at node \(j\). 
This temporal isolation occurs because each path has a unique delay line length determined by the sampling rate and the physical distances between nodes, resulting in minimal temporal overlap between arrivals during the early reflection phase. 
Therefore, the incoming wave vector to node \(j\), formed by the neighboring nodes can be represented as $\mathbf{p_j^+} = [\frac{1}{2}, 0, 0, 0, 0]^T$, 
where the nonzero value is at the index corresponding to the direction of the neighbor node \(k\). The specific placement of the index of node \(k\) is arbitrary and could be at any other index as each node has the same scattering matrix but with indices representing its current neighbors.
With the example vector $\mathbf{p_j^+}$, we are tracking one of the outgoing branches of node \(k\) which carries the pressure to node \(j\). At this instant where node \(j\) receives its only nonzero pressure transmitted to itself via node \(k\), all of the other incoming branches carry zero pressure. Consequently, $\mathbf{p_j^+}$ undergoes the scattering operation to determine the new outgoing pressures distributed from node \(j\) to its neighbors, as $\mathbf{p_j^-}=[-0.3, 0.2, 0.2, 0.2, 0.2]^T $. At the same time, $\mathbf{p_j^+}$ results in a node pressure of ${p_j}= \frac{1}{5}$, which is sent to the receiver. In summary, \(p_j= \frac{2}{N-1} \cdot \frac{p_{S}=1}{2} = \frac{1}{5}\) naturally appears due to the constant coefficient of the scattering matrix and the source injection factor as: 

\begin{equation}
\label{eq:second_order_pressure}
\underset{N \times N}{\begin{bmatrix}
    \scalebox{1.5}{$\mathbf{S}$} & \frac{1}{2}\mathbf{1} \\
    \frac{2}{5}\mathbf{1}^T & 1
\end{bmatrix}}
...
This calculation demonstrates how the source signal amplitude immediately drops from unit amplitude \(1 \cdot p_{S}\) to \(\frac{1}{5} \cdot p_{S}\) at the second reflections, the first instance where the SDN applies its recursive
distribution mechanism. During the very first pulses, incoming wave arrays typically contain only one non-zero elements, making the pressure tracking easier to validate upon examining the generated impulse responses excluding the wall and distance attenuations. \textcolor{purple}{
Figure~\ref{fig:rir_noabs} illustrates this behaviour for the
original SDN and two SW-SDN variants proposed in this paper (see \ref{sec:proposed_approach}).
}
All distance gains and wall absorptions are set to unity so that the only
attenuation present is the intrinsic decrease that arises
from the scattering operations and the architecture of the recursive network. The green rectangle encloses the six first reflections, whose amplitudes are $1\!\cdot\!p_S$, as expected due to removed attenuations. One recursion later
(yellow rectangle) the dominant second-order pulse appears with amplitude
$\tfrac{1}{5}\,p_S$, confirming the factor derived above.
At this very early stage each node receives only a single
non-zero incoming wave, making the $1/(N{-}1)$ drop clearly visible.
After a few more samples multiple branches begin to arrive
simultaneously, incoming arrays with multiple non-zero elements begin to appear. 
Because all absorptive mechanisms are disabled in this plot, the energy
injected by the impulse is perfectly conserved and recirculates
indefinitely among the nodes, producing the constant-level late tail
seen for the whole simulation duration.
...
%((For the specific room geometry under consideration, this temporal isolation of arrivals persists for approximately the first 24 reflections.)) 

A crucial insight out of this analysis is that, for the original SDN framework, the \(\frac{1}{N-1}\) pressure scaling factor occurs at the second order reflections (i.e the first outputs of the recursive part), indepedently from the room dimensions, source and receiver positions, absorptions, and distance attenuations.
The detailed analysis and numerical relationship between the source injection distribution and the energy circulating in the network is presented in Section \ref{sec:source_weighting_strategy}, which provided the motivation for the proposed approach.

Section~\ref{sec:energy_drop_problem} reveals fundamental phenomenon shaping the behavior of the SDN framework:

\textbf{The source injection} distributes the source signal equally to \(N-1\) incoming (and consequenty to the outgoing) directions, which effectively treats the source as if it were reflected from the neighboring walls. This implicitly assumes the wave has
already encountered all neighboring surfaces, and will reflect to all neighbors uniformly even at the first reflection, contrary to physical reality where initial reflections are predominantly specular. Lumping the source signal into all incoming directions is the way SDN manage to operates recursively. 
Even if the source signal is distributed non-uniformly or specularly, \textbf{the scattering matrix} will still transform the incoming waves into a non-fully-specular outgoing wave vectors.
This is because the scattering matrix is designed to redistribute energy among all directions, allowing it to simulate higher-order reflections without explicit path calculations. 
However, this redistribution lead to excessive energy loss in early reflections with the original source injection strategy.
Specifically, when only a single direction contains energy (as in early reflections), 
the redistribution necessarily reduces the amplitude in any single outgoing direction in an excessive manner,
resulting in energy drop immediately after first-order reflections, as observed in~\cite{SDNwaspaa}.
...
Consequently, the direct wave from the source is immediately spread out in SDN, rather than being directed towards a single specular reflection as would occur in a real room. 
In Chapter~\ref{sec:proposed_approach}, alternative source injection strategies are investigated and proposed to mitigate the issue to obtain a more realistic early reflection behavior, without sacrificing the diffuse late reverberation and the low-cost recursive nature of the SDN framework.

\section{Proposed Approach: Source-Weighted Injection (SW-SDN)}
\label{sec:proposed_approach}
Sec.~\ref{sec:energy_drop_problem} demonstrated that the standard SDN formulation, characterized by uniform source pressure injection~\eqref{eq:incoming_wave} and subsequent scattering~\eqref{eq:scattering_operation}, leads to a significant and geometry-independent pressure drop (scaling by 1/5 for any cuboid room) at the second-order reflections~\eqref{eq:second_order_pressure}. 
...
\textcolor{purple}{
\textbf{Constraint 2: Preserving the Target Direction.}
...
\textbf{Constraint 3: second-order pressure amplification.} If the initial source injection weighting is non-uniform (\( c \neq \text{Const} \cdot \mathbf{1} \)), the components of the first outgoing wave,
...
\begin{table*}[tbh]
    \centering
    \caption{Mean 50ms EDC-RMSE (dB) comparison of Room A. and Room W. Each room uses its own source and uniform receiver grid calculated according to **.  Values in parentheses denote SW-SDN results using random–target selection. \textbf{Reference}: ISM (rimpy-neg10).}
    ...
    \begin{tabular}{l ccccc | ccccc}
        \toprule
        & \multicolumn{5}{c|}{\textbf{Room A (Uniform Receivers)}} & \multicolumn{5}{c}{\textbf{Room Waspaa}} \\
...
\begin{figure*}[b]
    ...
    \caption{Spatial EDC RMSE contour maps for four source positions (rows), sixteen uniformly placed receivers and four SDN configurations $c=1,2,3,5$ (columns).  
...
The upper limit of the bound \(c<7\) follows directly from Constraint 2 in~\eqref{constraint3}\,(dominant–branch preservation), while the lower limit is anchored at the baseline \(c=1\). 
