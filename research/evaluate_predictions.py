...
# Error window for RMSE (milliseconds) – matches optimisation_singleC.py default
ERR_DURATION_MS = 50

# Whether to use the fast (basis-reconstruction) SDN path
USE_FAST_SDN = False

# Choose which existing data files to evaluate (comment/uncomment as needed).
# These must already exist in results/paper_data/ (generated by generate_paper_data.py).
FILES_TO_PROCESS = [
    # Quarter grid (legacy):
